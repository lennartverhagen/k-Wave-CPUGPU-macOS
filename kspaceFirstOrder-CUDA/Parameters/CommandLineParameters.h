 ...
 * first checks the <tt>OMP_NUM_THREADS</tt> variable. If it is not defined, the code uses the number of logical
 * processor cores. For the CUDA version, the number of threads is not critical as the CPU only preprocess data. When
 * running on desktops with multiple GPUs, or clusters with per-GPU resource allocations, it may be useful to limit the
 ...
 * the GPUs are set in the <tt>CUDA_DEFAULT</tt> mode, the first CUDA device is selected. In order to get the automatic
 ...
 * so no need to change it manually.
 *
 ...
 * checking the parameters of the simulation, code version, CPU/GPU used, file paths, and debugging running scripts,
 * verbose levels 1 and 2 may be very useful.
 *
 * The <tt>-h</tt> and <tt>\--help</tt> parameters print all the parameters of the CUDA/C++ code. The
 * <tt>\--version</tt> parameter reports detailed information about the code useful for debugging and bug reports. It
 * prints out the internal version, the build date and time, the git hash allowing us to track the version of the source
 * code, the operating system, the compiler name, and version and the instruction set used.
 *
